...
#' @returns NULL
#' @description
#' Calls awk. Note this writes all the given fasta-dts into ONE file, i.e., if you writeFasta(readFasta(someFastaFileNames)) you will concatenate all the someFastaFileNames into one output.
#' @export
writeFasta <- function(
    seqDT,
    outFastaFname = NULL,
  ...
  if(argNotGiven(outFastaFname)){outFastaFname <- paste0(deparse(substitute(seqDT)),".fasta",gzExt)}
  fileCon <- if(gzip==TRUE){gzfile(outFastaFname,open="w")}else{file(outFastaFname,open="w")}
  l_ply(1:nrow(seqDT),function(i){
    writeLines(paste0(">",seqDT[i,get(nameCol)]),fileCon)
    writeLines(paste0(seqDT[i,seq]),fileCon)
  })
